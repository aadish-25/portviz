...
window.addEventListener("message", (event) => {
  ...
  if (msg.type === "cliMissing") {
    showCliMissing(msg.error);
  }
  // ...existing handlers...
});

function showCliMissing(errorMsg) {
  const tabMessages = {
    overview: {
      title: "Portviz CLI Not Found",
      desc: "To use Portviz, install the Portviz CLI (Python package):",
      extra: errorMsg,
    },
    live: {
      title: "Portviz CLI Not Found",
      desc: "Live monitoring requires the Portviz CLI. Install it to see active ports and processes.",
      extra: errorMsg,
    },
    snapshots: {
      title: "Portviz CLI Not Found",
      desc: "Snapshots require the Portviz CLI to capture and compare port states.",
      extra: errorMsg,
    },
    orchestration: {
      title: "Portviz CLI Not Found",
      desc: "Orchestration requires the Portviz CLI to auto-detect and manage dev services.",
      extra: errorMsg,
    },
  };
  Object.entries(tabMessages).forEach(([tab, msg]) => {
    const el = document.getElementById("tab-" + tab);
    if (el) {
      el.innerHTML = `
        <div class="cli-missing">
          <svg class="cli-missing-icon" viewBox="0 0 24 24" width="64" height="64" fill="none" stroke="#29b6f6" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="12" cy="12" r="10"/>
            <line x1="2" y1="12" x2="22" y2="12"/>
          </svg>
          <h2>${msg.title}</h2>
          <div class="cli-desc">${msg.desc}</div>
          <div class="cli-install">
            <pre>pip install portviz</pre>
            <button class="btn-copy-pip">Copy</button>
            <div class="cli-reload-msg">After installing, press <b>Ctrl+Shift+P</b> and run <b>Reload Window</b> to activate Portviz.</div>
          </div>
          <div class="cli-error">${msg.extra}</div>
        </div>
      `;
    }
  });
  // Copy button handlers (attach to all visible buttons)
  document.querySelectorAll(".btn-copy-pip").forEach((b) => {
    b.onclick = (e) => {
      navigator.clipboard.writeText("pip install portviz");
